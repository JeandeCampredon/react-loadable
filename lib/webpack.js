  ...
  compilation.chunks.forEach(function (chunk) {
    chunk.files.forEach(function (file) {
      chunk.forEachModule(function (module) {
        ...
        var key = name && path.resolve(root, name).split('.').slice(0, -1).join('.');
        ...
        manifest[key].push({ id: id, name: name, file: file, publicPath: publicPath });
      });
    });
  });
...
var ReactLoadablePlugin = function () {
  ...
  ReactLoadablePlugin.prototype.apply = function apply(compiler) {
    ...
    compiler.plugin('emit', function (compilation, callback) {
      var manifest = buildManifest(compiler, compilation, _this.root);
